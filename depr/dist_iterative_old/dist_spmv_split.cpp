    ...
    hipblasHandle_t &default_cublasHandle)
{
    ...
    for(int i = 0; i < size; i++){
        ...
        if(i > 0){
            cudaErrchk(hipStreamWaitEvent(default_stream, A_subblock.events_recv_subblock[i-1], 0));
        }
        if(i > 0){
            cublasErrchk(hipblasDgemv(
                default_cublasHandle,
        ...
        if(i < size-1){
            ...
            cudaErrchk(hipMemcpyAsync(
                ...
                hipMemcpyHostToDevice, A_subblock.streams_recv_subblock[i]));

            cudaErrchk(hipEventRecord(A_subblock.events_recv_subblock[i],
