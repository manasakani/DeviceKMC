    ...
    double *time_taken)
{
    ...
    // first straight forward implementation
    ...
    double a, b, na;
    double alpha, beta, alpham1, r0, r_norm2_h;
    size_t bufferSize = 0;
    void *buffer = NULL;
    ...
    int k = 1;
    while (r_norm2_h/norm2_rhs > relative_tolerance * relative_tolerance && k <= max_iterations) {
        if(k > 1){
            // pk+1 = rk+1 + b*pk
            b = r_norm2_h / r0;
    ...
    double *time_taken)
{
    ...
    // for memcpy
    // manually copy into pinned memory of ddot
    ...
    double *time_taken)
{
    // replaces new with cudaMallocHost
    // for memcpy
    // ddot and others are not manually copied
    ...
    while (r_norm2_h/norm2_rhs > relative_tolerance * relative_tolerance && k <= max_iterations) {
        ...
        cublasErrchk(cublasDdot(cublasHandle, rows_per_rank, p_local_d, 1, Ax_local_d, 1, &dot_h));
        //allreduce        
        MPI_Allreduce(MPI_IN_PLACE, &dot_h, 1, MPI_DOUBLE, MPI_SUM, comm);
    ...
    double *time_taken)
{
    ...
    while (r_norm2_h/norm2_rhs > relative_tolerance * relative_tolerance && k <= max_iterations) {
        ...
        //allgather
        //memcpy
        MPI_Allgatherv(p_local_d, rows_per_rank, MPI_DOUBLE, p_d, recvcounts, displs, MPI_DOUBLE, comm);
        cusparseErrchk(cusparseSpMV(
            cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA_local, vecp,
            &beta, vecAp_local, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, buffer));
