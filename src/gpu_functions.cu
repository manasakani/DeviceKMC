...
__global__ void calculate_pairwise_interaction(const T* posx, const T* posy, const T*posz, 
                                          const double *lattice, const int pbc, const int N, T* result_array){
    ...
    for (int idx = tid_total; idx < N * N; idx += num_threads_total) {
        ...
        int j = idx % N;
        double dist = 0.0;
        dist = site_dist_gpu(posx[i], posy[i], posz[i], posx[j], posy[j], posz[j], lattice[0], lattice[1], lattice[2], pbc);
